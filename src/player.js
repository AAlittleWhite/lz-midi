...
(function () {
  ...
  player.loadMidiFile = function (onsuccess, onprogress, onerror) {
    try {
      const midiFileData = MidiFile(player.currentData);
      player.replayer = new Replayer(midiFileData, player.timeWarp, null, player.BPM)
  ...
  var startAudio = function (currentTime, fromCache, onsuccess) {
    ...
    for (var n = 0; n < length && messages < 100; n++) {
      ...
      var isA = obj[0] && obj[0].event 
                && (obj[0].event.subtype === "noteOn" || obj[0].event.subtype === "noteOff") 
                && obj[0].event.noteNumber >= 0;
