...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	var requestQueue = function requestQueue(opts, context) {
	  var audioFormat = opts.format; // mp3
	  var instruments = opts.instruments; // ['acoustic_grand_piano']
	  var onprogress = opts.onprogress;
	  var onerror = opts.onerror;
	  // /
	  var length = instruments.length; // 1
	  var pending = length;
	...
	*/
	...
	(function () {
	  ...
	  player.loadMidiFile = function (onsuccess, onprogress, onerror) {
	    try {
	      var midiFileData = (0, _jasmid.MidiFile)(player.currentData);
	      player.replayer = new _jasmid.Replayer(midiFileData, player.timeWarp, null, player.BPM);
	      player.data = player.replayer.getData();
	      player.endTime = getLength();
	      // console.log('原始数据', player.data)
	      // console.log('555原始数据', player.data)
	      // /
	   ...
	   * @param {*} fromCache 是否从缓存中播放
	   * @param {*} onsuccess 播放成功的回调函数
	   * @returns 
	   */
	  var startAudio = function startAudio(currentTime, fromCache, onsuccess) {
	    ...
	    for (var n = 0; n < length && messages < 100; n++) {
	      ...
	      var isA = obj[0] && obj[0].event && (obj[0].event.subtype === "noteOn" || obj[0].event.subtype === "noteOff") && obj[0].event.noteNumber >= 0;
